    ...
    public async Task<LeadImportResultDto> ImportAsync(Stream stream, LeadActor actor, CancellationToken cancellationToken = default)
    {
        ...
        for (var i = 0; i < rows.Count; i++)
        {
            ...
            var request = new LeadUpsertRequest(
                ...
                Email: LeadCsvImportHelper.ReadValue(row, "email"),
                ...
                Source: LeadCsvImportHelper.ReadValue(row, "source"),
                Territory: territory,
                AutoScore: LeadCsvImportHelper.ReadBool(row, "autoscore"),
                Score: LeadCsvImportHelper.ReadInt(row, "score") ?? 0,
                AccountId: null,
                ContactId: null,
