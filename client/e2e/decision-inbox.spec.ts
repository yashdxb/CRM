...
test('decision inbox route and dashboard task stream render', async ({ page, request }) => {
  await login(page, request);

  await page.goto('/app/dashboard', { waitUntil: 'domcontentloaded' });
  await expect(page.getByText('Decision Inbox').first()).toBeVisible();
  await expect(page.getByRole('button', { name: /Decisions \(/i })).toBeVisible();

  await page.goto('/app/decisions', { waitUntil: 'domcontentloaded' });
