...
const DEFAULT_LEAD_DATA_WEIGHTS = [
...
const QUALIFICATION_FACTOR_SCORE_CONFIG = [
...
export function isMeaningfulQualificationValue(value) {
...
export function computeLeadDataQualityScore(input, configuredWeights) {
    ...
    const earnedWeight = normalizedWeights.reduce((sum, item) => {
        const enabled = active[item.key] ?? false;
        return sum + (enabled ? item.weight : 0);
    }, 0);
    return clampScore(Math.round((earnedWeight / totalWeight) * 100));
}
export function computeQualificationRawScore(input, configuredFactors) {
    ...
    if (!hasAnyMeaningfulFactor)
        return null;
    const groups = ['C', 'Q', 'V', 'S'];
...
export function computeLeadScore(input, configuredLeadDataWeights, configuredQualificationFactors, configuredLifecycleWeights) {
    ...
    const finalLeadScore = activeWeightTotal > 0
...
function computeHistoryExecutionScore(input) {
...
function normalizeLifecycleWeights(configuredWeights) {
    return {
...
function normalizeQualificationFactors(configuredFactors) {
...
function normalizeLeadDataWeights(configuredWeights) {
