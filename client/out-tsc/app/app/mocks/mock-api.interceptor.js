...
import { buildDashboardSummary, createRole, createUser, deleteRole, deleteUser, findRole, findUser, getWorkspaceSettings, updateWorkspaceSettings, getPermissionDefinitions, lookupActiveUsers, listRoles, isUserAudienceRoleAssignmentAllowed, mockCustomers, resetUserPassword, searchActivities, searchCustomers, searchOpportunities, searchUsers, setUserActiveStatus, updateRole, updateUser, createOpportunity, getOpportunityById, updateOpportunity, deleteOpportunity, searchProperties, getPropertyById, createProperty, updateProperty, deleteProperty, getPriceHistory, addPriceChange, getShowings, createShowing, updateShowing, getDocuments, addDocument, deleteDocument, getActivities, createActivity, updateActivity, getMlsFeeds, createMlsFeed, triggerMlsImport, getMlsImportHistory, getCmaReport, getSignatureRequests, createSignatureRequest, getAlertRules, createAlertRule, toggleAlertRule, getAlertNotifications, searchContacts } from './mock-db';
...
export const mockApiInterceptor = (req, next) => {
    const path = normalizePath(req.url);
    if (!environment.useMockApi || !path.startsWith('/api')) {
        return next(req);
    }
    const respond = (body, status = 200, delayMs = 120) => of(new HttpResponse({ status, body })).pipe(delay(delayMs));
    ...
    if (req.method === 'POST' && path === '/api/properties/mls-feeds') {
        return respond(createMlsFeed(req.body), 201, 140);
    }
    ...
    if (req.method === 'GET' && path.startsWith('/api/properties')) {
        ...
        const search = req.params.get('search') ?? undefined;
        const result = searchProperties({ page, pageSize, status: status, propertyType: propertyType, city, search });
        return respond(result, 200, 140);
    }
    ...
    if (req.method === 'PUT' && /^\/api\/properties\/[^/]+\/showings\/[^/]+$/.test(path)) {
        ...
        const updated = updateShowing(showingId, req.body);
    ...
    if (req.method === 'PUT' && /^\/api\/properties\/[^/]+\/activities\/[^/]+$/.test(path)) {
        ...
        const updated = updateActivity(actId, req.body);
        return respond(updated ?? { message: 'Not found' }, updated ? 200 : 404, 100);
    }
    // ── CMA Reports (G3) ──
    ...
    if (req.method === 'GET' && path === '/api/dashboard/summary') {
        const period = req.params.get('period') ?? 'month';
    ...
    if (req.method === 'PUT' && path === '/api/workspace') {
        return respond(updateWorkspaceSettings(req.body), 200, 140);
    }
