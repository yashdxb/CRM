...
const ownerIdFor = (name) => (name ? ownerIds[name] : undefined);
let mockWorkspaceSettings = {
    id: 'tenant-default',
    key: 'default',
    name: 'CRM Enterprise',
    timeZone: 'UTC',
...
export function searchCustomers(query) {
    ...
    if (searchTerm) {
        result = result.filter((c) => [c.name, c.company, c.email, c.phone].some((field) => field.toLowerCase().includes(searchTerm)));
    }
...
const stageToStatus = (stage) => {
    if (!stage)
        return 'Open';
    if (stage.startsWith('Closed Won'))
        return 'Closed Won';
    if (stage.startsWith('Closed Lost'))
        return 'Closed Lost';
    return 'Open';
};
...
export function searchOpportunities(query) {
    ...
    if (searchTerm) {
        result = result.filter((opp) => [opp.name, opp.account, opp.owner].some((value) => value.toLowerCase().includes(searchTerm)));
    }
    const total = result.length;
    const items = paginate(result, query.page ?? 1, query.pageSize ?? 10);
    return { items, total };
...
export function buildDashboardSummary(period = 'month', fromUtc, toUtc) {
    const now = new Date();
...
const externalAudienceRestrictedPermissions = new Set([
...
const paginateUsers = (items, page = 1, pageSize = 10) => {
    const start = (page - 1) * pageSize;
    return items.slice(start, start + pageSize);
};
...
export const updateWorkspaceSettings = (payload) => {
...
export const searchUsers = (query) => {
    ...
    if (searchTerm) {
        result = result.filter((user) => [user.fullName, user.email].some((value) => value.toLowerCase().includes(searchTerm)));
    }
    const total = result.length;
    const items = paginateUsers(result, page, pageSize).map(toUserListItem);
    return { items, total };
};
export const lookupActiveUsers = (search, max = 200) => {
...
export const isUserAudienceRoleAssignmentAllowed = (audience, roleIds) => {
...
export const setUserActiveStatus = (id, nextStatus) => {
    const record = mockUsers.find((user) => user.id === id);
    if (!record) {
        return false;
    }
...
export const resetUserPassword = (_id) => true;
// ── Contact Mock Data ──
const mockContacts = [
...
export function searchContacts(query) {
    ...
    if (searchTerm) {
        result = result.filter((c) => [c.name, c.email, c.accountName].filter(Boolean).some((f) => f.toLowerCase().includes(searchTerm)));
    }
    const total = result.length;
...
export function searchProperties(query) {
    ...
    if (searchTerm) {
        result = result.filter((p) => [p.address, p.city, p.mlsNumber, p.neighborhood, p.ownerName, p.accountName]
            .filter(Boolean)
...
export function createProperty(payload) {
    ...
    const record = {
        ...
        propertyType: payload.propertyType || 'Detached',
...
export function updateProperty(id, payload) {
    ...
    target.propertyType = payload.propertyType || target.propertyType;
...
export function addPriceChange(pc) {
    const record = { id: `pc-${Math.random().toString(36).slice(2, 8)}`, ...pc };
...
export function getShowings(propertyId) {
    return mockShowings
        .filter((s) => s.propertyId === propertyId)
        .sort((a, b) => new Date(b.scheduledAtUtc).getTime() - new Date(a.scheduledAtUtc).getTime());
}
export function createShowing(s) {
    const record = { id: `sh-${Math.random().toString(36).slice(2, 8)}`, ...s, createdAtUtc: new Date().toISOString() };
    mockShowings.unshift(record);
    return { ...record };
}
export function updateShowing(id, updates) {
...
export function getActivities(propertyId) {
    return mockPropertyActivities
        .filter((a) => a.propertyId === propertyId)
        .sort((a, b) => new Date(b.createdAtUtc).getTime() - new Date(a.createdAtUtc).getTime());
}
export function createActivity(a) {
    const record = { id: `act-${Math.random().toString(36).slice(2, 8)}`, ...a, createdAtUtc: new Date().toISOString() };
...
export function getMlsFeeds() {
    return [...mockMlsFeeds];
}
export function createMlsFeed(feed) {
    const record = {
...
export function triggerMlsImport(feedId) {
...
export function getMlsImportHistory() {
...
function generateCmaComparables(propertyId) {
    return mockComparables[propertyId] || mockComparables['prop-001'];
}
function buildCmaSummary(comps) {
...
export function getCmaReport(propertyId) {
...
export function getSignatureRequests(propertyId) {
    return mockSignatureRequests
        .filter(s => s.propertyId === propertyId)
        .sort((a, b) => new Date(b.createdAtUtc).getTime() - new Date(a.createdAtUtc).getTime());
}
export function createSignatureRequest(req) {
    const record = {
...
export function getAlertRules(propertyId) {
...
export function toggleAlertRule(id, isActive) {
...
export function getAlertNotifications(propertyId) {
